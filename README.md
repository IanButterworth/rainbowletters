...
English, Spanish, Swedish, French, German, Portuguese and Italian. Pick one from the small row of names on the start screen; the choice is remembered, and there is deliberately no way to change it once play begins. Each language has its own letter pictures (in Spanish, G pops a gato), its own words for the emoji showers, its own on-screen text, and a matching speech voice. Words match without accents, so typing MAMA finds MAMÁ and the accented spelling is what gets spoken. Everything per language lives in `languages.js`, so adding one is a matter of copying a block.
