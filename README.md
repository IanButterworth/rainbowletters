...
English, American English, Spanish, Swedish, French, German, Portuguese and Italian. English is British: the letters and words are read by a British voice, and American English is the same game in an American accent (so Z is "zed" or "zee"). Pick one from the small row of names on the start screen; the choice is remembered, and there is deliberately no way to change it once play begins. Each language has its own letter pictures (in Spanish, G pops a gato), its own words for the emoji showers, its own on-screen text, and a matching speech voice. A voice from the right region is preferred over any other voice for the language, and one installed on the machine over one that needs the network. English prefers a female British voice (Kate or Serena if they are installed, otherwise whichever the machine has), and an Enhanced or Premium download is used ahead of the compact voice of the same name. Words match without accents, so typing MAMA finds MAMÁ and the accented spelling is what gets spoken. Everything per language lives in `languages.js`, so adding one is a matter of copying a block.
